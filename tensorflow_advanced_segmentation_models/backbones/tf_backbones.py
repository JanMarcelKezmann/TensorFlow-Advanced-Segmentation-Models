...
def create_base_model(name="ResNet50", weights="imagenet", height=None, width=None,
    ...
    if name.lower() == "densenet121":
        ...
        if height <= 31 or width <= 31:
            raise ValueError(
                "Parameters 'height' and 'width' should not be smaller than 32.")
        base_model = tf.keras.applications.MobileNet(include_top=include_top, weights=weights, input_shape=input_shape,
                                                     pooling=pooling, alpha=alpha, depth_multiplier=depth_multiplier, dropout=dropout)
        ...
        base_model = tf.keras.applications.NASNetMobile(
            include_top=include_top, weights=weights, input_shape=input_shape, pooling=pooling)
        layer_names = ["zero_padding2d_4", "cropping2d_5",
                       "cropping2d_6", "cropping2d_7", "activation_838"]
